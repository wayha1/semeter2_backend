...
function UserTable() {
  ...
  const fetchData = async () => {
    try {
      const token = '3|5QIq1gghCG6gHDSDtG093Vb47OpWRUwmqES4IB6u67677603';
      ...
      <table className="table-auto w-full border-collapse border border-gray-500">
        <thead>
          <tr className="bg-gray-200">
            ...
            <th className="border border-gray-500 p-2">Email Verified At</th>
            <th className="border border-gray-500 p-2">Google ID</th>
            <th className="border border-gray-500 p-2">Gender</th>
            <th className="border border-gray-500 p-2">Is Active</th>
            <th className="border border-gray-500 p-2">User Image</th>
            <th className="border border-gray-500 p-2">Phone Number</th>
            <th className="border border-gray-500 p-2">User Address</th>
            <th className="border border-gray-500 p-2">Status</th>
            <th className="border border-gray-500 p-2">Created At</th>
            <th className="border border-gray-500 p-2">Updated At</th>
            <th className="border border-gray-500 p-2">Actions</th>
          </tr>
        </thead>
        <tbody className='text-white p-2 text-center'>
          {userData.map(user => (
            <tr key={user.id}>
              ...
              <td className="border border-gray-500 p-2">
                <button onClick={() => handleEdit(user)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded mr-2">
                  Edit
                </button>
                {/* Delete button */}
                <button
