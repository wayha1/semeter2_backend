...
function ProductNameElysian() {
  ...
  const fetchData = async () => {
    try {
      ...
      const ElysianProducts = response.data.data.filter(
        (product) => product.product_brand === "Elysian Skincare 2023"
      );
      setProduct1(ElysianProducts);

      // Set brand name if products are available
      if (ElysianProducts.length > 0) {
        setBrandName(ElysianProducts[0].product_brand);
      }
    } catch (error) {
...
export default ProductNameElysian